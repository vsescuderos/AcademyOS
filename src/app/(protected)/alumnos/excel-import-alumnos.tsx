...
async function parseFile(file: File): Promise<{ rows: ParsedAlumno[]; errors: string[] }> {
  const XLSX = await import("xlsx");
...
export default function ExcelImportAlumnos({
  onSuccess,
  onCancel,
}: {
  onSuccess: () => void;
  ...
  function handleImport() {
    startTransition(async () => {
      const res = await crearAlumnosBatch(rows);
      setResult(res);
    ...
    >
      ...
      {rows.length > 0 && !result && (
        <div style={{ marginTop: 12 }}>
          <p style={{ fontSize: 12.5, color: "var(--t2)", marginBottom: 8 }}>
            {rows.length} alumnos listos para importar:
          </p>
          <table
      ...
      {result && (
        <div style={{ marginTop: 12 }}>
          ...
          >
            {result.created} alumno{result.created !== 1 ? "s" : ""} importado
